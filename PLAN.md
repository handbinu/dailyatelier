...
비로그인 사용자가 보호 화면이나 작품 상세의 로그인 필요 작업에서 로그인한 뒤 기존
내부 위치로 돌아가 흐름을 계속할 수 있게 한다. pathname, search와 hash는 보존하되
외부 URL과 잘못된 복귀 값은 홈으로 대체한다. 작가 역할 guard, 작가 마이페이지 메뉴
개편과 로그인 전 실패한 POST 요청의 자동 재실행은 이번 범위에서 제외한다.

### 1단계 — 안전한 복귀 위치 계약

- [x] React Router location state에서 pathname, search와 hash를 안전한 내부 경로로
      변환하는 공통 계약을 정의한다.
- [x] 문자열·location 객체 형태를 지원하되 절대 URL, protocol-relative URL,
      인증 화면 순환과 유효하지 않은 값은 `/`로 대체한다.
- [x] 직접 로그인 화면에 진입했거나 복귀 정보가 없으면 기존 홈 이동을 유지한다.
- [x] 복귀 경로는 로그인 성공 뒤 한 번만 소비하고 localStorage에 장기 보존하지 않는다.

### 2단계 — 로그인 유도 경로 연결

- [x] `PrivateRoute`가 전달한 현재 위치를 `Login`에서 소비해 로그인 성공 후 복귀한다.
- [x] 작품 상세의 입찰·찜 로그인 유도가 같은 복귀 계약을 사용하게 한다.
- [x] 주문 등 API 401 후 로그인으로 이동하는 화면은 현재 GET 화면 위치를 전달하되
      실패한 변경 요청을 로그인 후 자동 재실행하지 않는다.
- [x] 로그아웃과 사용자가 직접 선택한 로그인 이동은 의도하지 않은 이전 위치를
      재사용하지 않게 한다.
- [x] 인증 정보 저장과 auth-state 변경 알림의 기존 동작을 유지한다.

### 3단계 — 회귀 검증과 문서

- [x] 복귀 경로 정규화, 보호 라우트, 작품 상세 로그인 유도, API 401과 기본 홈 이동을
      컴포넌트 테스트로 검증한다.
- [x] 외부 URL, protocol-relative URL, 인증 화면 순환과 손상된 state가 홈으로
      대체되는지 검증한다.
- [x] lint, 단위·컴포넌트 테스트, production build와 `git diff --check`를 통과한다.
- [x] 브라우저에서 보호 URL의 query·hash 복귀, 작품 상세 작업 복귀와 인증 해제 후
      현재 화면 복귀를 확인하고 외부 복귀 값 차단은 컴포넌트 테스트로 검증한다.
- [x] 검증 후 로그인 위치 복귀 BACKLOG 항목만 완료 처리하고 작가 역할 guard와 주문
      UI 관련 항목은 미완료로 유지한다.

### 완료 기준

- 로그인 성공 후 안전한 원래 내부 위치의 pathname, search와 hash가 복원된다.
- 복귀 정보가 없거나 안전하지 않으면 홈으로 이동하며 로그인 화면 순환이 발생하지 않는다.
- 인증 만료 뒤 재로그인해도 현재 GET 화면으로 돌아오며 이전 실패 요청은 재실행되지 않는다.
- 직접 로그인, 로그아웃, 기존 인증 저장·해제와 공개 페이지 동작에 회귀가 없다.

### 검증 결과

- ESLint, 단위 테스트 15개, 컴포넌트 테스트 78개와 production build가 통과했고
  `git diff --check` 오류가 없다.
- 브라우저에서 보호된 주문 URL의 query·hash 복귀, 작품 상세 로그인 복귀, 직접 로그인
  시 홈 이동과 다른 탭에서 인증이 해제된 뒤 현재 주문 화면 복귀를 확인했다.
- 절대 URL, protocol-relative URL, 백슬래시 URL, 인증 화면 순환과 손상된 location
  state는 컴포넌트 테스트에서 홈으로 대체됨을 확인했다.
- 로그인 전 실패한 요청의 자동 재실행과 작가 역할 guard는 구현하지 않았다.

### 커밋 경계

1. `30c21d6` `fix(auth): 로그인 후 안전한 원래 위치 복귀`
2. `chore: 로그인 복귀 검증 결과 정리`

복귀 계약, 연결 코드와 직접 관련된 테스트는 첫 번째 기능 커밋에 포함한다. `PLAN.md`의
계획·검증 상태와 `BACKLOG.md` 완료 표시는 전체 검증 후 두 번째 문서 커밋에서만 처리한다.
