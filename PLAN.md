...
> [!IMPORTANT]
> **현재 진행 단계:** 0단계 — 기존 구현 조사와 개발 계획 확정 완료, 1단계 구현 시작 전
>
...
- [ ] 명시적 DB 마이그레이션 방식을 도입하고 운영 DDL 자동 변경을 제한한다.
- [ ] `PointAccount`, `PointTransaction`, `PointHold`, `PointCharge`와 관련 enum을 추가한다.
- [ ] 저장소, 유니크 제약과 조회 인덱스를 추가한다.
- [ ] 기존 `User.reserve`를 `PointAccount.availableBalance`로 이관한다.
- [ ] 기존 잔액마다 `OPENING_BALANCE`를 한 번만 생성한다.
- [ ] 프로필 조회를 `PointAccount` 기준으로 변경하고 `User.reserve` 직접 쓰기를 금지한다.

검증:

- [ ] 신규 사용자 포인트 계정 초기화
- [ ] 기존 잔액 이관과 마이그레이션 재실행 멱등성
- [ ] 음수 잔액·금액 차단과 유니크 제약
- [ ] 실제 MySQL 스키마와 계정·원장 합계 대조

### 2단계 — 충전 상태와 내부 결제 어댑터

- [ ] 충전 생성, 내부 승인, 실패, 취소, 환불 상태 전이를 구현한다.
- [ ] `merchantOrderId`, `pgOrderId`, 멱등성 키와 결제 금액 검증을 구현한다.
- [ ] `PaymentProvider` 인터페이스와 권한이 제한된 `INTERNAL` 구현체만 추가한다.
- [ ] 충전 완료와 `CHARGE` 원장·잔액 증가를 같은 트랜잭션으로 처리한다.
- [ ] 충전 환불을 `REFUND` 반대 거래로 처리한다.
- [ ] 콜백 inbox와 중복 이벤트 처리 계약을 구현한다.

검증:

- [ ] 모든 허용·금지 상태 전이
- [ ] 중복 승인·취소·환불
- [ ] 같은 멱등성 키의 동일 요청과 다른 요청
- [ ] 금액 불일치와 PG 주문번호 중복
- [ ] 원장 저장 실패 시 충전 상태·잔액 롤백
- [ ] 실패 이벤트 재처리와 최대 재시도

### 3단계 — 입찰 예치와 차액 재입찰

- [ ] 기존 입찰 트랜잭션에 포인트 계정과 hold를 연결한다.
- [ ] 신규 최고 입찰 금액 전액을 예치한다.
- [ ] 같은 최고 입찰자의 재입찰은 차액만 추가 예치한다.
- [ ] 새 사용자가 최고가가 되면 이전 사용자의 예치를 해제한다.
- [ ] 패찰 후 재입찰은 새 hold로 전액 예치한다.
- [ ] 잔액 부족 오류를 추가한다.
- [ ] 입찰·예치·이전 예치 해제·현재가 변경을 한 트랜잭션으로 처리한다.
- [ ] 작품 취소 시 활성 예치를 같은 트랜잭션에서 해제한다.

검증:

- [ ] 신규 입찰, 동일 사용자 증액과 패찰 후 재입찰
- [ ] 잔액 전액 입찰과 1포인트 부족
- [ ] 입찰·원장·계정 저장 실패별 전체 롤백
- [ ] 취소와 입찰 경합
- [ ] 동시 입찰의 잔액 초과·이중 예치·이중 해제 방지
- [ ] 서로 다른 작품에서 같은 사용자 잔액을 동시에 사용하는 경우

### 4단계 — 자동 마감과 낙찰 주문 결제

- [ ] 마감 시 최고 입찰과 활성 hold 정합성을 검증한다.
- [ ] 낙찰 주문에 결제수단 `INTERNAL_POINT`를 저장한다.
- [ ] `COMMIT`과 주문 `PAID`를 원자 처리한다.
- [ ] 결제 전 포기와 기한 만료 시 예치를 해제한다.
- [ ] 결제 후 환불 시 반대 거래로 포인트를 복원한다.
- [ ] 마감·결제·만료·환불 재실행을 멱등 처리한다.
- [ ] 기존 `OrderPaymentService` 경계를 포인트 결제 유스케이스에 맞게 조정한다.
...
- [ ] 결제 동시 호출의 중복 확정 방지
- [ ] 환불 동시 호출의 중복 복원 방지
- [ ] 주문 상태 변경 실패 시 포인트 변경 롤백
- [ ] 포인트 처리 실패 시 주문 `PAYMENT_PENDING` 유지

### 5단계 — API와 프론트 연동

- [ ] 프로필·마이페이지에 사용 가능 포인트와 예치 포인트를 구분해 제공한다.
- [ ] 포인트 거래 내역과 충전 내역·상태 조회 API를 추가한다.
- [ ] 입찰 API에 잔액 부족과 예치 결과를 필요한 범위만 제공한다.
- [ ] 주문 결제 API에 `Idempotency-Key`를 적용한다.
- [ ] `Charge.jsx` 목업을 제거하고 내부 충전 흐름에 연결한다.
- [ ] 주문 화면 결제 버튼을 실제 포인트 결제 API에 연결한다.
- [ ] 중복 클릭을 방지하고 `409` 발생 시 최신 잔액·주문을 다시 조회한다.
- [ ] 네이버페이·토스페이는 실제 동작 가능한 결제수단처럼 노출하지 않는다.
