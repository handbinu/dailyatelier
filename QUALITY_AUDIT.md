...
| QA-010 | 중간 | 확정 결함 | 충전 초기 조회 전·실패 후에도 0원 잔액과 충전 가능 상태를 표시함 | P1 | 해결 — 로딩 상태 구현 및 정상 충전 브라우저 검증 완료 |
| QA-011 | 낮음 | 확정 결함 | 이전 중복확인 응답이 변경된 회원가입 입력을 검증 완료로 표시함 | P2 | 해결 — 응답 경합 방지 및 컴포넌트 검증 완료 |
| QA-012 | 낮음 | 잠재 위험 | 실패한 충전 후 금액을 바꿔도 이전 멱등성 키를 재사용함 | P2 | 해결 — 요청 키 수명 자동 검증 완료, 응답 유실 브라우저 검증은 미실행 |
| QA-013 | 중간 | 확정 결함 | 도메인별 예외 처리 범위가 달라 오류 응답 계약이 일관되지 않음 | P1 | 해결 — 공통 예외 계약 및 회귀 검증 완료 |
| QA-014 | 낮음 | 잠재 위험 | 포인트 목록의 음수 페이지가 명시적인 400 API 오류로 변환되지 않음 | P2 | 해결 — 페이지 경계 검증 및 회귀 검증 완료 |
...
- 일반 회원으로 실제 충전 화면에 진입해 초기 조회 중 잔액 자리표시자와 충전 버튼 비활성화, 조회 완료 후 잔액과 데모 충전 안내 표시를 확인했다.
- 50,000P 데모 충전 후 잔액, `PAID` 충전 내역과 `데모 포인트 충전` 거래 내역이 함께 갱신됐고, 화면 재조회와 새로고침 후에도 잔액 50,000P·충전 1건·원장 1건으로 유지돼 중복 적립이 없음을 확인했다.
- 조회 실패 후 재조회 동작은 이번 브라우저 검증에서 장애를 주입하지 않아 미실행으로 유지한다.
...
- 공통 중복확인 훅에서 요청 식별자와 요청 당시 입력값을 함께 확인해 현재 입력과 일치하는 최신 응답만 검증 상태에 반영하도록 변경했다.
- 입력 변경 시 진행 중 요청을 무효화하고 검증 상태와 메시지를 초기화하며, 확인 중이거나 미확인 상태에서는 회원가입 제출을 차단한다.
- 일반·작가 회원가입 각각에서 지연 응답, 응답 순서 역전, 실패 후 동일 값 재확인, 서버 중복 거부와 정상 제출을 포함한 컴포넌트 테스트 8건을 추가했다.
- 전체 프론트 유틸리티 테스트 11건과 컴포넌트 테스트 14건, ESLint와 production build가 통과했다. 전체 백엔드 테스트는 235건 중 실패·오류 0건, 제외 7건이었다.
- 브라우저에서 일반·작가 계정을 실제 생성하고, 입력 변경 시 확인 상태 초기화, 미확인 상태 제출 차단과 늦은 실패 응답이 최신 입력을 덮지 않는 것을 확인했다.
- 늦은 성공 응답 경합은 로컬 응답이 빨라 브라우저에서 재현하지 못했으며, 요청 순서 역전을 포함한 컴포넌트 테스트 결과를 근거로 유지한다.
...
- 후속 `QA-014` 구현 전 기준으로 포인트 음수 페이지 검증은 이 커밋 범위에 포함하지 않았다.
...
- 거래 내역과 충전 내역 조회에서 `page < 0`, `size < 1`, `size > 50`을 저장소 호출 전에 거부하도록 변경했다.
- 잘못된 페이지 요청은 `INVALID_POINT_REQUEST` 코드와 `timestamp`, `status`, `code`, `message`, `path` 필드를 갖는 400 응답으로 변환한다.
- 두 목록의 음수 page, size 0, size 51과 정상 기본 page·size 및 상한 50 요청을 API·서비스 테스트로 검증했다.
- 전체 백엔드 테스트 235건이 통과했고 실패·오류 0건, 제외 7건이었다. 프론트 유틸리티 테스트 11건과 컴포넌트 테스트 6건, ESLint와 production build도 통과했다.
- `QA-011`은 이 커밋 범위에 포함하지 않았고 후속 커밋 `b42ecd7`에서 별도로 해결했다.

##### 후속 브라우저·로컬 API 검증 결과

- 거래·충전 내역의 `page=0`, `size=20` 요청이 각각 200과 1건을 반환하는 정상 조회를 확인했다.
- 두 목록 모두 `page=-1`, `size=0`, `size=51` 요청이 `timestamp`, `status`, `code`, `message`, `path` 필드와 `INVALID_POINT_REQUEST` 코드를 가진 400 응답을 반환했다.

### 주문 브라우저 회귀 범위

- 자동 테스트에서는 주문 상태 전이, 권한·충돌 응답, 환불 원장 처리와 주문 ID별 중복 요청 가드를 검증했다.
- 주문 0건 상태의 구매자 주문 목록과 작가 판매 주문 관리 화면 진입, 통계와 빈 상태 표시를 먼저 확인했다.
- 이후 환불 주문 1건으로 구매자 목록·상세의 환불 완료 표시, 판매자 목록·상세의 승인·거절 작업 미노출과 새로고침 후 상태 유지를 확인했다.
- 구매자·판매자 목록·상세 API는 모두 `REFUNDED`, `APPROVED`, 가능한 작업 빈 목록을 반환했다.
- 배송 완료, 구매 확정, 환불 요청·승인·거절 실행과 중복 재요청은 전체 브라우저 흐름으로 검증하지 않았으므로 미실행 상태를 유지한다.

### 환불 요청 완료 상태 전이 후속 처리

- 상태: 해결
- 구현 커밋: `76e7db9 fix(backend): 환불 요청 완료 상태 전이 추가`
- 승인 시 주문 `REFUNDED`와 환불 요청 `APPROVED`를 함께 보장하고 `REQUESTED → APPROVED`로 전이한다.
- 승인 완료 후 동일 승인과 거절 완료 후 동일 거절은 멱등 성공하며, 완료 상태에서 반대 작업은 충돌로 차단한다.
- `V2__normalize_approved_refund_requests.sql`은 기존 `REFUNDED + REQUESTED` 행만 `APPROVED`로 보정하며 로컬 Flyway 적용을 확인했다.
- 전체 백엔드 테스트 236건은 실패·오류 0건, 제외 7건이었다. 포인트 `REFUND` 원장 1회 생성과 중복 잔액 복원 방지, 구매자·판매자 목록·상세 응답 일관성을 함께 검증했다.
- 브라우저와 API 최종 대조가 통과했으며 구매자 목록의 `환불` 문구 개선은 BACKLOG로 유지한다.
