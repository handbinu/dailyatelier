...
- 위 무발견 근거는 `QA-004`의 실제 처리기 트랜잭션 경계, `QA-005`의 의미적 정합성 범위와 `QA-006`의 동시 최초 수신을 제외한 범위다.
...
- 실제 PG 도입 전 `QA-004`의 트랜잭션 실패 경계와 `QA-006`의 동시 최초 수신을 MySQL 통합 테스트로 확정해야 한다.
